...
def seed_executions(db, rule: models.Rule, version: models.RuleVersion, days: int = 90):
    # Delete existing demo executions to reseed fresh data
    existing = (
    ...
    if existing > 0:
        print(f"Deleted {existing} existing demo executions for rule {rule.rule_id}. Reseeding...")
    db.commit()
    ...
    for day_idx in range(days):
        ...
        for i in range(total_claims):
            ...
            # More realistic decisions: triggered rules more likely to be fraud, non-triggered more likely legitimate
            if execution_result:
                # Triggered rules: higher chance of being fraud, some pending for review
                decision_choice = random.choices(
                    [models.Decision.fraud, models.Decision.pending, models.Decision.legitimate],
                    weights=[6, 3, 1],  # 60% fraud, 30% pending, 10% legitimate
                    k=1,
                )[0]
            else:
                # Non-triggered rules: mostly legitimate, some pending
                decision_choice = random.choices(
                    [models.Decision.legitimate, models.Decision.pending, models.Decision.fraud],
                    weights=[8, 2, 0],  # 80% legitimate, 20% pending, 0% fraud
                    k=1,
                )[0]
...
def main():
    ...
    try:
        ...
        inserted = seed_executions(db, rule, version, days=90)
        print("Seed complete.")
